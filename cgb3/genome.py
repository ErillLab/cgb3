...
class Genome:
    ...
    """
    def __init__(self, strain_name, accession_numbers, use_old_locus_tags):
        ...
        self._chromids = [Chromid(acc, self, use_old_locus_tags) for acc in accession_numbers]
    ...
    def build_PSSM_model(self, collections, weights):
        ...
        seqs = [seq[start:start+model.length] for seq in noncoding_regions
                for start in range(len(seq)-model.length)]
