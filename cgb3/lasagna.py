...
def ComputeFreqs(sites, SCOPE, alphabet='acgt-', bgCounts=None, counts=None,
    ...
    freqs = numpy.array([{'a': 0.0, 'c': 0.0, 'g': 0.0, 't': 0.0, '-': 0.0}\
                         for i in range(l)]) # a dict at each position
    for i in range(l):
...
def PSSM(sites, SCOPE, alphabet='acgt-', bgCounts=None, counts=None,
         counts_2mer=None, ICThres=0.0, covThres=0.4,
         A=True):
...
def AddGaps(aligned, jSeq, jPSSM,
            bgCounts=None, counts=None, counts_2mer=[],
        ...
        if counts_2mer != [] and SCOPE > 0:
            ...
            for scope in range(1, SCOPE + 1):
                if len(counts_2mer[scope - 1]) == 0:
                    ...
                    counts_2mer[scope - 1] = [{} for k in range(nLeft)] + counts_2mer[scope - 1] + [{} for k in range(nRight)]
            for i in range(nAligned - 1):
                for scope in range(1, SCOPE + 1):
                    for k in list(range(min(nLeft, l - scope))) + list(range(max(nLeft, nLeft + PSSML - scope), l-scope)):
...
def LASAGNA(sites, SCOPE, seedIdx=-1, ICThres=0.0, covThres=0.4):
...
def ReadSeq(path):
    ...
    inp.close()
    for seq in tmp:
        lines = list(map(str.strip, seq.split('\n')))
        headers.append(lines[0])
        seqs.append(''.join(lines[1:]))
    return headers, seqs

def LASAGNA_alignment(seqs):
    aligned, idxAligned, strands = LASAGNA(seqs,0)
    aligned = returnTrimmedSequences(aligned)

    return(aligned)
    
def returnTrimmedSequences(aligned):   
    bool_position=[True for item in range(len(aligned[0]))]
    range_align=[]
    trimmed_sequences=["" for item in range(len(aligned))]
    for seq in aligned:
        for i,letter in enumerate(seq):
            if letter=="-":
                bool_position[i]=False

    for i,item in enumerate(bool_position):
        if item==True:
            range_align.append(i)
    
    for j,seq in enumerate(aligned):
        for i in range_align:
            trimmed_sequences[j]=trimmed_sequences[j]+seq[i]
