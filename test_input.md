...
- `use_old_locus_tags`
    ...
    - Default value: `true`
    - Rationale:
    	- Determines whether CGB will preferentially use old_locus_tag fields (in RefSeq chromid records) over the locus_tag field. This is convenient so that the results of using RefSeq records are consistent with available literature on genes (predominantly represented by the GenBank (old) locus_tag identifier.
